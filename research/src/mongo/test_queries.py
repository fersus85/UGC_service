...
def measure_scenario(
    ...
    times = [measure_func(db, obj_id) for obj_id in random.choices(ids, k=iterations)]
...
def test_queries(dsl: Dict, iterations: int = 100) -> None:
    ...
    total_time_fav, avg_time_fav = measure_scenario(db, user_ids, measure_user_favorites, iterations)
    total_time_rating, avg_time_rating = measure_scenario(db, movie_ids, measure_movie_average_rating, iterations)

    logging.info("Сценарий «список закладок»:")
    logging.info("  Общее время для %s итераций: %s", iterations, total_time_fav)
    logging.info("  Среднее время на один запрос: %s", avg_time_fav)

    logging.info("Сценарий «средняя оценка фильма»:")
    logging.info("  Общее время для %s итераций: %s", iterations, total_time_rating)
