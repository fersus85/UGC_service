...
class MongoRepository:
    ...
    async def insert_one(
        ...
        try:
            ...
            if await self.find_one(collection_name, document):
                logger.exception(
                    f'Entry for User: {document["user_id"]} '
                    + f"in the {collection_name}: already exists"
                )
                return None
            ...
            logger.info(
                f'User: {document["user_id"]} added an entry '
                + f"to the collection: {collection_name}"
            )
            return str(insert_one_result.inserted_id)
        except Exception as ex:
            logger.exception(
                "Error when adding an entry "
                + f"to the collection {collection_name}: {ex}"
            )
            return None
    ...
    async def find_one(
        ...
        try:
            ...
            logger.exception(
                "Error when searching for an entry "
                + f"in the {collection_name}: {ex}"
            )
            return None
    ...
    async def find_all(
        ...
        try:
            ...
            logger.exception(
                "Error when searching for an entry "
                + f"in the collection: {collection_name}: {ex}"
            )
            return None
        if entries:
            logger.info(
                "Entries in the collection: "
                + f"{collection_name} found {len(entries)}",
            )
            return entries
        ...
        try:
            ...
            if update_result:
                logger.info(
                    ...
                    + f"{collection_name} updated successfully"
                )
    ...
    async def delete_one(
        ...
        try:
            ...
            if await self.find_one(collection_name, query):
                ...
                logger.info(
                    f'User: {query["user_id"]} an entry was deleted '
                    + f"from the collection: {collection_name}"
                )
                return delete_result.deleted_count
            logger.info(
                f'Entry for User: {query["user_id"]} '
                + f"in the collection: {collection_name} not found"
            )
            return None
        except Exception as ex:
            logger.exception(
                "Error when deleting an entry "
                + f"from a collection: {collection_name}: {ex}"
            )
            return None
